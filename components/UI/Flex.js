...
export const Flex = ({
	...
	if (dir) {
		flexAttr = `display: ${isInline ? 'inline-flex' : 'flex'};`;
		...
		<div className={classnames(['Flex', { [className]: className }])}>
			...
			<style jsx>{`
				...
				@media screen and (max-width: ${responsiveWidth || '800px'}) {
					.Flex {
						flex-direction: ${responsiveCollapse ? 'column' : responsiveCollapseReverse ? 'column-reverse' : dir};
